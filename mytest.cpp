...
int main() {
  ...
  JQueue* jobQueue1 = new JQueue(priorityFn1); //dynamically allocating obeject

  // Create some test jobs and insert them into the queue
  
  job_t testJob1("test1", 46, 32, 10,  14, 1023, 119);
  job_t testJob2("test2", 49, 6, 4, 123, 510, 297);
  job_t testJob3("test3", 21, 3,  0, 128, 1024, 1800);
  job_t testJob4("test1a", 49, 3, 11,  16, 1024, 120);

  jobQueue1 -> insertJob(testJob1);
  jobQueue1 -> insertJob(testJob2);
  jobQueue1 -> insertJob(testJob3);
  jobQueue1 -> insertJob(testJob4);
  ...
  jobQueue1 -> setPriorityFn(priorityFn2);
  ...
  jobQueue1 -> setPriorityFn(priorityFn1);
  ...
  JQueue* jobQueue2 = new JQueue(*jobQueue1);

  std::cout << "Deleting jobQueue1" << std::endl;
  delete jobQueue1;

  std::cout << "Printing out jobQueue2 again..." << std::endl;
  jobQueue2 -> dump();
  std::cout << "\n";
